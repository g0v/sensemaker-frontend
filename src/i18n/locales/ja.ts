export default {
  ...
  home: {
    ...
    additionalContextLabel: '📝 追加コンテキスト:',
  ...
  selfHost: {
    ...
    viewBackendProject: 'バックエンドプロジェクトを表示',
    ...
    viewFrontendProject: 'フロントエンドプロジェクトを表示',
  ...
  guideModal: {
    ...
    polisTitle: '1. 意見調査レポートのエクスポート',
    polisDescription: 'このツールは、Polisおよび Agora プラットフォームからエクスポートされた意見調査レポートをサポートしています。CSVデータファイルが正しいデータ構造に準拠していることを確認してください。',
    polisGuideLink: 'Polis使用ガイドを表示 →',
    agoraGuideLink: 'Agora Citizen使用ガイドを表示 →',
  ...
  // Agoraガイドページ
  agoraGuide: {
    title: '「Agora Citizen 市民審議プラットフォーム」使用ガイド',
    subtitle: 'V1 · 対応言語：英語、スペイン語、フランス語、簡体字中国語、繁体字中国語、日本語、アラビア語',
    mvpNote: '本ガイドは MVP 版の説明です。現在、公式サイトは正式版に更新されています。',
    mainWebsite: 'メインサイト：',
    officialVideo: '公式紹介動画：',
    youtubeLink: 'YouTubeリンク',
    focusTitle: '焦点',
    focusBeforeLink: '民主主義の大きな課題は、参加者が増えると直接民主主義的な市民審議や意見の集約がほぼ不可能になることです。いわゆる「寡頭制の鉄則」——大規模では民主主義は代議士とエリートの少数支配になりがちです。Agora Citizenは、社会の分極化を乗り越え、その限界を突破する非同期型市民審議プラットフォームで、',
    focusLinkText: 'ZKorumチーム',
    focusAfterLink: 'が開発しています。',
    focusSuffix: '商業モデルで運営され、オープンソースとしてソースコードを公開しています。',
    step0Title: '「Try the MVP」をクリックして始める',
    step0Description: 'メインサイトにアクセス後、「Try the MVP」をクリックするとMVP（最小実行可能製品）を利用できます。',
    step0LinkText: 'Agora Citizenメインサイトへ',
    step1Title: 'アカウント登録',
    step1Login: '右上の「ログイン」ボタンをクリックしてください。',
    step1Register: '「登録」ボタンをクリックしてください。',
    step1Next: '説明を読んだら「矢印」をクリックして次へ進んでください。',
    step2Title: '電話番号認証',
    step2Phone: '「電話番号で認証」を選択してください。',
    step2Verify: '電話番号を入力したら「矢印」をクリック。認証コードを入力すると認証が完了します。',
    step3Title: '基本プロフィールの入力',
    step3Bullet1: '入室後、まずユーザー名の入力が求められます（小文字の英数字とアンダースコアのみ）。',
    step3Bullet2: '次に、使用できる言語を選択します。',
    step3Bullet3: '最後に、関心のあるトピック（例：芸術と文化）を選択します。複数選択可。',
    step4Title: 'トピックに参加',
    step4Enter: 'まずトピックをクリックして入室します。',
    step4ScrollBefore: '下にスクロールすると多くの意見が表示されます。翻訳ツール（例：',
    step4ScrollLink: 'Google翻訳',
    step4ScrollAfter: '）で内容を理解するのに役立てられます。',
    voteTitle: '投票',
    voteDescription: '各意見に対して「賛成」「反対」「スキップ」で投票できます。投票後は他者の投票数・割合が表示されます。最初は独立した判断を促すため非表示です。',
    reportTitle: '分析レポートを見る',
    reportTabsDesc: '各会話には「意見」と「分析」の2つのタブがあります。タブをクリックして切り替えます。',
    reportFourDimensions: '分析レポートには4つの項目があります：要約、共通点、相違点、グループ。',
    reportSummary: '要約',
    reportSummaryDesc: '共通点・相違点・グループをまとめて表示します。',
    reportCommon: '共通点',
    reportCommonDesc: 'グループを問わず多くの参加者が賛成している意見に焦点を当て、「稀な合意」を可視化します。',
    reportDivergence: '相違点',
    reportDivergenceDesc: '争点やグループ間の意見の対立に焦点を当てます。',
    reportGroups: 'グループ',
    reportGroupsDesc: 'AIが投票者をクラスタ分けします。各グループがどの意見に賛成・反対しているか（青：賛成多め、黄：反対多め）が分かります。データが十分あれば自分がどのグループに属するかも表示されます。',
    publishTitle: '意見を投稿',
    publishDescription: '会話に自分の意見を投稿することもできます。投稿した意見は自動的に「賛成」として設定されます。',
    step5Title: 'トピックを立ち上げる',
    step5Description: '左下の「会話を開始」をクリックすると新しいトピックを立ち上げられます。',
    etymologyTitle: 'Agoraの語源',
    etymologyBefore: 'プラットフォーム名は古代ギリシャの市民広場',
    etymologyLink: 'Agora',
    etymologyAfter: 'に由来します。哲学者ソクラテスが対話を重ねた場所でもあります。',
    supportTitle: 'サポート・機能要望・バグ報告',
    supportDiscussionLabel: '利用の議論：',
    supportDiscussionLinkText: 'vTaiwan審議民主Slackチャンネル',
    supportIssuesLabel: '機能要望・バグ報告：',
    supportIssuesLinkText: '親プロジェクトのIssues（GitHub）',
    backToHome: 'ホームに戻る',
    nextGuide: 'Open Routerガイド'
  },
